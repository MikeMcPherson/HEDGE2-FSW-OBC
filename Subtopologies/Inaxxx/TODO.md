...
Add the `Inaxxx/` folder to its parent directory's `CMakeLists.txt` file if not already there:

```cmake
add_fprime_subdirectory("${CMAKE_CURRENT_LIST_DIR}/Inaxxx/")
```
...
### Required Files:
- **`Inaxxx.fpp`**: Define your subtopology's instances and connections
- **`InaxxxConfig/InaxxxConfig.fpp`**: Configure constants, queue sizes, stack sizes, and priorities
- **`PingEntries.hpp`**: Define ping entries for health monitoring of components (if needed)

### Optional Files (add as needed):
- **`InaxxxConfig/InaxxxSubtopologyConfig.cpp/.hpp`**: 
  - Add C++ configuration code (e.g., memory allocators, custom initialization)
  - Update `InaxxxConfig/CMakeLists.txt` to include:
    ```cmake
    register_fprime_config(
        SOURCES
            "${CMAKE_CURRENT_LIST_DIR}/InaxxxSubtopologyConfig.cpp"
        HEADERS
            "${CMAKE_CURRENT_LIST_DIR}/InaxxxSubtopologyConfig.hpp"
        AUTOCODER_INPUTS
            "${CMAKE_CURRENT_LIST_DIR}/InaxxxConfig.fpp"
    )
    ```

- **`InaxxxConfig/Additional*.fpp`**: 
  - Add additional FPP configuration files (e.g., ComDriverConfig.fpp for communication subtopologies)
  - Include them in the `InaxxxConfig/CMakeLists.txt` AUTOCODER_INPUTS

- **`InaxxxTopologyDefs.hpp`**: 
...
## 4. Common Patterns
- **Queue/Stack/Priority Configuration**: Define in `InaxxxConfig.fpp`
...
- **Component Configuration**: Add additional FPP files in config `InaxxxConfig` 